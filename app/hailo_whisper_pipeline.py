                ...
                with decoder_infer_model.configure() as decoder_configured_infer_model:
                    ...
                    while self.running:
                        try:
                            ...
                            # Decoder - force Japanese transcription prefix
                            # <|startoftranscript|>=50258 <|ja|>=50266 <|transcribe|>=50359 <|notimestamps|>=50363
                            import os as _os
                            _pp=_os.environ.get("WHISPER_PROMPT","").strip()
                            forced_prefix=([50361]+self.tokenizer.encode(" "+_pp)) if _pp else []
                            forced_prefix=forced_prefix+[50258, 50266, 50359, 50363]
                            prefix_len = len(forced_prefix)
                            decoder_input_ids = np.zeros((1, self.decoding_sequence_length), dtype=np.int64)
                            for _p, _t in enumerate(forced_prefix):
                                decoder_input_ids[0][_p] = _t

                            generated_tokens = []
                            decoder_outputs = None
                            # Run Decoder Iteratively (generation starts after the forced prefix)
                            for i in range(prefix_len - 1, self.decoding_sequence_length - 1):
